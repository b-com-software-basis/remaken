...
std::pair<std::string, fs::path> MesonGeneratorBackend::generate(const std::vector<Dependency> & deps, Dependency::Type depType)
{
    ...
    fos<<prefix<<"_incdir = include_directories("<<cflags<<")"<<std::endl;
...
void MesonGeneratorBackend::generateIndex(std::map<std::string,fs::path> setupInfos)
{
    fs::detail::utf8_codecvt_facet utf8;
...
void MesonGeneratorBackend::generateConfigureConditionsFile(const fs::path &  rootFolderPath, const std::vector<Dependency> & deps)
{
    fs::detail::utf8_codecvt_facet utf8;
...
void MesonGeneratorBackend::parseConditionsFile(const fs::path &  rootFolderPath, std::map<std::string,bool> & conditionsMap)
{
    fs::detail::utf8_codecvt_facet utf8;
